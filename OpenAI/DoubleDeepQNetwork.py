...
class DDQNAgent():
    ...
    def learn(self, obs):
        ...
        # Don't learn until memory has at least batch_size samples
        self.mem_cntr += 1
        ...
        # Finding Double Q targets
        acts = self.q(torch.Tensor(next_states)).argmax(dim=1)
        q_next = self.target(torch.Tensor(next_states))[batch_index, acts]
        q_next[dones] = 0.0
        q_target = torch.tensor(rewards) + self.gamma*q_next
...
ddqn_agent = DDQNAgent(env, gamma=0.99, epsilon=1.0, batch_size=64, 
                 n_actions=3, decay=0.999, input_dims=[6], lr=0.001)

ddqn_agent.train(195, num_episodes=350, C=5, verbose=True, plot=False)
final_benchmark(env, ddqn_agent, 10, render=True)
